import { Settings, FileCheck, Calendar, Cpu, Star, HelpCircle, Layers, CheckCircle } from "lucide-react";
import { SEO } from "../../components/SEO";
import { Button } from "../../components/ui/Button";

export default function LegoIstituti() {
  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "Course",
    "name": "Lego Spike Robotica per istituti scolastici",
    "description": "Progetto strutturato di robotica educativa Lego Spike per scuole, dalla classe pilota al progetto d'istituto annuale.",
    "provider": {
      "@type": "Organization",
      "name": "Digital Heroes",
  ...
  return (
    <div className="w-full flex-1 flex flex-col gap-20 md:gap-28 p-4 sm:p-8 md:p-12 lg:p-16 bg-brand-bg text-[#1a1025]">
      <SEO
        title="Lego Spike per Istituti | Digital Heroes"
        description="Robotica educativa come progetto d'istituto. PNSD, STEM e PTOF con gestione a nostro carico, kit Lego inclusi."
        jsonLd={jsonLd}
      />
      ...
      <section className="relative bg-white border-2 border-brand-secondary p-8 sm:p-16 md:p-20 rounded-3xl shadow-[6px_6px_0_0_#1a1025] flex flex-col items-center text-center overflow-hidden mt-28 md:mt-36 max-w-6xl mx-auto w-full">
        <div className="relative z-10 max-w-4xl space-y-8 flex flex-col items-center">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-black uppercase leading-[1.05] tracking-tight text-[#1a1025]">
            Robotica educativa come progetto d'istituto
          </h1>
          
          <p className="text-lg md:text-xl font-medium max-w-2xl mx-auto text-[#1a1025]/80">
            Lego Spike porta la robotica in classe in modo concreto: costruzione, programmazione e pensiero computazionale, con gestione completa a nostro carico dalla classe pilota al progetto annuale.
          </p>

          <div className="pt-4 flex flex-wrap justify-center gap-3.5 items-center">
            <span className="bg-brand-bg border-2 border-brand-secondary px-4 py-2 rounded-full text-xs font-bold text-[#1a1025] uppercase shadow-[2px_2px_0_0_#1a1025]">9+ Età consigliata</span>
            <span className="bg-brand-bg border-2 border-brand-secondary px-4 py-2 rounded-full text-xs font-bold text-[#1a1025] uppercase shadow-[2px_2px_0_0_#1a1025]">Kit su richiesta</span>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto w-full space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            {
              title: "Kit Lego Spike (opzionali)",
              desc: "Portiamo noi tutto il materiale se l'istituto non ne è provvisto.",
              icon: <Cpu className="w-8 h-8 text-orange-500" />
            },
            {
              title: "Calendario flessibile",
              desc: "Da un singolo workshop a un percorso distribuito sull'anno scolastico.",
              icon: <Calendar className="w-8 h-8 text-emerald-500" />
            },
            {
              title: "Documentazione per il PTOF",
              desc: "Report finale utilizzabile per rendicontazione e piano triennale dell'offerta formativa.",
      ...
      <section className="max-w-5xl mx-auto w-full bg-orange-50 border-2 border-brand-secondary p-8 sm:p-12 rounded-3xl shadow-[6px_6px_0_0_#1a1025] space-y-8">
        <div className="text-center mb-8">
          <h2 className="text-3xl md:text-4xl font-black uppercase tracking-tight">Conformità e organizzazione</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            "Educatori formati su STEM e robotica educativa, esperienza diretta in aula.",
            "Percorso adattabile a PNSD, PCTO e progetti STEM dell'istituto.",
            "Possibilità di coinvolgere più classi con calendario condiviso o in rotazione."
          ].map((text, idx) => (
            <div key={idx} className="flex flex-col gap-4 items-center text-center bg-white p-8 border-2 border-brand-secondary rounded-2xl shadow-[4px_4px_0_0_#1a1025] hover:-translate-y-1 hover:shadow-[6px_6px_0_0_#ff9f1c] transition-all">
              <CheckCircle className="w-10 h-10 text-orange-500 shrink-0" />
      ...
      <section className="max-w-6xl mx-auto w-full space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-[#cc7cf7] text-white border-2 border-brand-secondary p-8 rounded-2xl shadow-[4px_4px_0_0_#1a1025]">
            <Settings className="w-10 h-10 text-yellow-300 mb-6 animate-spin-slow" />
      ...
      <section className="max-w-6xl mx-auto w-full text-center">
        <h3 className="text-2xl font-black uppercase mb-6">Cosa portano a casa gli studenti</h3>
        <div className="flex flex-wrap justify-center gap-3">
          {["Pensiero computazionale", "Progettazione meccanica", "Programmazione a blocchi", "Problem solving", "Lavoro di squadra"].map(tag => (
      ...
      <section className="max-w-4xl mx-auto w-full space-y-8">
        ...
        <div className="space-y-6">
          {[
            {
              q: "Serve uno spazio specifico per la robotica?",
              a: "No, basta un'aula normale con tavoli liberi. Portiamo noi tutto il materiale necessario."
            },
            {
              q: "Il progetto rientra nei fondi STEM o PNSD?",
              a: "Sì, il percorso è strutturabile per rientrare nelle linee di finanziamento per la didattica STEM."
            },
            {
      ...
      <section className="max-w-4xl mx-auto w-full text-center pb-12">
        <h2 className="text-3xl md:text-4xl font-black uppercase tracking-tight mb-6">
          Porta la robotica nel tuo istituto
        </h2>
